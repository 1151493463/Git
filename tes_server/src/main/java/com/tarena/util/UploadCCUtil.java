...
public class UploadCCUtil {
	 ...
	 * @return
	 */
	public String getPlayCode(Map<String,String> paramsMap){
		paramsMap.put("userid", ccKeyUtil.getUserId());
		String playCode = null;
		Map<String,Map<String,String>> contentMap = null;
		String content = "";
		String playCodeHost = "http://spark.bokecc.com/api/video/playcode";
		long time = System.currentTimeMillis();
		String requestUrl = QueryStringUtil.createHashedQueryString(paramsMap, time, ccKeyUtil.getAppKey());
		try{
			//发送创建视频的请求
			HttpClient httpClient=HttpClients.createDefault();
			HttpGet httpGet=new HttpGet(playCodeHost+"?"+requestUrl);
			...
			if(content!=null){
				contentMap=(Map<String,Map<String,String>>)JSON.parse(content.trim());
				playCode = contentMap.get("video").get("playcode");
			}
			//System.out.println("create contentMap-->"+contentMap);
		}catch(Exception e){
			e.printStackTrace();
